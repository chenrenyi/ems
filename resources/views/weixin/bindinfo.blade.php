...
@section('content')
     <div class="container">
      ...
      <form action="{{ URL('weixin/saveinfo') }}"  method="POST" class="form-signin">
        ...
        <input name="name" type="text" id="inputEmail" class="form-control" placeholder="姓名" required autofocus>
        <label for="inputPassword" class="sr-only">学号</label>
        <input name="number" type="text" id="inputPassword" class="form-control" placeholder="学号" required>
        <div class="checkbox">
          <label>
          请保证信息正确，绑定后不可再更改
          </label>
